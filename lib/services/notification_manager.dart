...
class NotificationManager {
  ...
  void scheduleNextNotification(List<BaseTodo> allTodoRules) {
    _notificationTimer?.cancel();

    final now = DateTime.now();
    final List<TodoInstance> upcomingInstances = [];
    final lookAheadRange = DateTimeRange(start: now, end: now.add(const Duration(days: 7)));
      ...
      if (todoRule is SingleTodo) {
        if (!todoRule.dateTime.isBefore(lookAheadRange.start) &&
          ...
          if (todoRule.dateTime.isAfter(now)) {
            upcomingInstances.add(TodoInstance.fromTodo(todoRule, todoRule.dateTime));
          }
        }
      } else if (todoRule is RecurringTodoRule) {
        for (var day = lookAheadRange.start;
    ...
    if (upcomingInstances.isEmpty) {
      print('Không còn công việc nào trong tương lai gần để lên lịch thông báo.');
      return;
    }

    upcomingInstances.sort((a, b) => a.concreteDateTime.compareTo(b.concreteDateTime));
    ...
    if (!delay.isNegative) {
      print('Sẽ thông báo cho ${notificationsToShow.length} sự kiện vào lúc: $nextTimestamp, sau $delay');
      ...
      _notificationTimer = Timer(delay, () {
        ...
        scheduleNextNotification(allTodoRules);
      });
    }
  }
